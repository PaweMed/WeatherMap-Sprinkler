...
public:
  ...
  // *** LOGIKA PROCENTOWA – PRIORYTET OPADÓW 6h, przy 0mm przechodzimy do reguł T/H ***
  // Deszcz (ostatnie 6h):
  //   rain6h ≥ 4.0 mm          -> 0% (anuluj podlewanie)
  //   1.0 mm ≤ rain6h < 4.0 mm -> 40%
  //   0.0 mm < rain6h < 1.0 mm -> 80%
  //   rain6h == 0.0 mm         -> reguły T/H (jak dotychczas)
  //
  // Reguły T/H (bieżące, gdy rain6h == 0.0):
  //   gorąco i sucho: T_now > 27°C && H_now < 50%  -> 120%
  //   chłodno/wilgotno: H_now > 70%                -> 80%
  //   w pozostałych przypadkach                    -> 100%
  int getWateringPercent() {
    const float rain6h = getLast6hRain();
    const float T_now  = temp;
    const float H_now  = humidity;

    // 1) Priorytet opadów 6h
    if (rain6h >= 4.0f) return 0;
    if (rain6h >= 1.0f) return 40;
    if (rain6h >  0.0f) return 80;

    // 2) Brak opadów -> reguły T/H (bieżące)
    if (T_now > 27.0f && H_now < 50.0f) return 120; // gorąco i sucho
    if (H_now > 70.0f)                   return 80;  // wilgotno/chłodno
    return 100; // neutralnie
  }
  ...
  String getWateringDecisionExplain() {
    ...
    // Sekcja opadów (priorytet)
    if (rain6h >= 4.0f)
      return "Opady z 6h = " + String(rain6h,1) + " mm (≥ 4.0) → 0% – podlewanie anulowane";
    if (rain6h >= 1.0f)
      return "Opady z 6h = " + String(rain6h,1) + " mm (1.0–<4.0) → 40%";
    if (rain6h > 0.0f)
      return "Opady z 6h = " + String(rain6h,1) + " mm (0.0–<1.0) → 80%";

    // Brak opadów -> reguły T/H (bieżące)
    if (T_now > 27.0f && H_now < 50.0f)
      return "Brak opadów 6h i gorąco/sucho (T=" + String(T_now,1) + "°C >27, H=" + String(H_now,0) + "% <50) → 120%";
    if (H_now > 70.0f)
      return "Brak opadów 6h i chłodno/wilgotno (H=" + String(H_now,0) + "% >70) → 80%";

    return "Brak opadów 6h i warunki neutralne (T=" + String(T_now,1) + "°C, H=" + String(H_now,0) + "%) → 100%";
  }
};
