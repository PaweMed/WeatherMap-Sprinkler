...
public:
  ...
  void publishStatus(bool force = false) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      // POPRAWKA #12: publikuj także pozostały czas (sekundy)
      int remaining = zones ? zones->getRemainingSeconds(i) : 0;
      String remStr = String(remaining);
      mqttClient.publish((topic("zones/" + String(i) + "/remaining")).c_str(), remStr.c_str(), true);
    }
