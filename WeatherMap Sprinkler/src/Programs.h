...
class Programs {
  ...
  Config*         config   = nullptr; // NOWE – wskaźnik na Config
...
public:
  void begin(Zones* z, Weather* w, Logs* l, PushoverClient* p, Config* c) {
    ...
    logs = l;
    pushover = p;
    config = c; // przypisanie wskaźnika Config
    loadFromFS();
  }
  ...
  void loop() {
    ...
    for (int i = 0; i < numProgs; i++) {
      ...
      if (nowMins == progMins && (P.lastRun == 0 || lastYday != nowTm.tm_yday)) {
        int actualDuration = P.duration;
        int wateringPercent = weather ? weather->getWateringPercent() : 100;

        if (wateringPercent == 0) {
          if (logs) logs->add("Podlewanie odwołane - warunki pogodowe!");
          if (pushover && config && config->getEnablePushover()) pushover->send("Podlewanie odwołane - warunki pogodowe!");
          continue;
        } else {
          actualDuration = (actualDuration * wateringPercent) / 100;
          if (logs) logs->add("Podlewanie dostosowane do " + String(wateringPercent) + "% (" + String(actualDuration) + "min)");
          if (pushover && config && config->getEnablePushover()) {
            pushover->send("Podlewanie dostosowane do " + String(wateringPercent) + "% (" + String(actualDuration) + "min)");
          }
        }
