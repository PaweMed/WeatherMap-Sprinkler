...
class Weather {
  ...
  RainHistory rainHistory; // NOWE – historia opadów
...
public:
      ...
      if (apiKey.isEmpty() || location.isEmpty()) {
        ...
        if (code > 0) {
          ...
          if (!err) {
            ...
            // NOWE – dodanie do historii opadów
  ...
  // NOWE – API dla WebServerUI
  void rainHistoryToJson(JsonDocument& doc) const {
    rainHistory.toJson(doc);
  }
  ...
  int getWateringPercent() {
    ...
    if (rain6h >= 5.0) return 0;
    if (rain6h >= 2.0 && rain6h < 5.0) return 40;
    return 100;
  }
